...
(function (factory) {
	...
	let BEFORE_CLOSE_EVENT = 'BeforeClose';
	let AFTER_CLOSE_EVENT = 'AfterClose';
	let BEFORE_APPEND_EVENT = 'BeforeAppend';
	let MARKUP_PARSE_EVENT = 'MarkupParse';
	let OPEN_EVENT = 'Open';
	let CHANGE_EVENT = 'Change';
	let NS = 'mfp';
	let EVENT_NS = `.${NS}`;
	let READY_CLASS = 'mfp-ready';
	let REMOVING_CLASS = 'mfp-removing';
	let PREVENT_CLOSE_CLASS = 'mfp-prevent-close';
	...
	let _getEl = function (className, appendTo, html, raw) {
	...
	let _mfpTrigger = function (e, data) {
	...
	let _getCloseBtn = function (type) {
	...
	$.fn.magnificPopup = function (options) {
		...
		if (typeof options === 'string') {
			if (options === 'open') {
				let items;
				let itemOpts = _isJQ ? jqEl.data('magnificPopup') : jqEl[0].magnificPopup;
				let index = parseInt(arguments[1], 10) || 0;
	...
	let _putInlineElementsBack = function () {
	...
	let AJAX_NS = 'ajax';
	let _ajaxCur;
	let _removeAjaxCursor = function () {
		if (_ajaxCur) {
			$(document.body).removeClass(_ajaxCur);
		}
	};
	let _destroyAjaxRequest = function () {
		_removeAjaxCursor();
		if (mfp.req) {
			mfp.req.abort();
		}
	...
	/* >>zoom*/
	let hasMozTransform;
	let getHasMozTransform = function () {
	...
	$.magnificPopup.registerModule('zoom', {
		...
		proto: {
			...
			initZoom() {
				...
				let getElToAnimate = function (image) {
					let newImg = image.clone().removeAttr('style').removeAttr('class').addClass('mfp-animated-image');
					let transition = `all ${zoomSt.duration / 1000}s ${zoomSt.easing}`;
					let cssObj = {
						'position': 'fixed',
						'zIndex': 9999,
						'left': 0,
	...
	let _emptyPage = '//about:blank';

	let _fixIframeBugs = function (isShowing) {
	...
	let _replaceCurrTotal = function (text, curr, total) {
	...
	$.magnificPopup.registerModule('gallery', {
		...
		proto: {
			initGallery() {
				...
				_mfpOn(`BuildControls${ns}`, () => {
					if (mfp.items.length > 1 && gSt.arrows && !mfp.arrowLeft) {
						...
						let arrowLeft = mfp.arrowLeft = $(markup.replace(/%title%/gi, gSt.tPrev).replace(/%dir%/gi, 'left')).addClass(PREVENT_CLOSE_CLASS);
						let arrowRight = mfp.arrowRight = $(markup.replace(/%title%/gi, gSt.tNext).replace(/%dir%/gi, 'right')).addClass(PREVENT_CLOSE_CLASS);
			...
			preloadNearbyImages() {
				...
				let preloadBefore = Math.min(p[0], mfp.items.length);
				let preloadAfter = Math.min(p[1], mfp.items.length);
				let i;
